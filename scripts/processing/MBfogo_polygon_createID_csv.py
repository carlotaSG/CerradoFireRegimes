...
def cleanData(polygon_fp):
    ...
    # UPDATE (20/10/2023) Comment out this line if running after assigning 
    # month to polygons that are merged in space
    # polygon_gdf = polygon_gdf.rename(columns = {'raster_val':'month'})
    # Convert to integer
    polygon_gdf['month'] = polygon_gdf['month'].astype(int)
...
def main():
    ...
    in_dir = '../../data/processed/MBfogo_c20_cerrado_polygons_b150_CM/'
    ...
    list_fp = fileList.selecting_fp_withCondition(list_fp, [str(i) for i in range(1985, 2022 + 1)])
