...
def landuse(data, min_cat, max_cat):
    ...
    data = data[['id', 'anthropic', 'natural', 'flammable', 'nonflammable', '4', '3', '12', '11', '15', '18']]
...
def main():
    ...
    for year in range(start_year, end_year + 1):
        ...
        noncat = [0, 2, 6, 7, 8, 16, 17, 28, 34, 35, 37, 38, 42, 43, 44, 45]
    
        t.tic()
        print('Counting pixels per grid cell and category')
        lu = assignRasterValues_toPolygon(grid_fp, lu_fp.format(year), 0, 49, nodata = 99, non_categories = noncat, num_process = 10)
        t.toc('Pixels per land use counted in')
        
        # 2. Discarding columns not of interest
        if grid_size == '50':
            lu = pd.DataFrame(lu.drop(columns = ['left', 'top', 'right', 'bottom', 'inner_cell', 'adj_amazon', 'adj_caatin', 'adj_pantan', 'adj_mata-a', 'area_EPSG5', 'withinCerr','%area_crop', 'clipArea', 'geometry']))
        else:
            lu = pd.DataFrame(lu.drop(columns = ['left', 'top', 'right', 'bottom', 'withinCerr','clipArea', 'geometry']))
            
        # Filling in MB land use categories that have sub-categories
        lu = mbtools.fill_classes(lu, 6)
        
        
        # 2. Calculating area per cell and land-use category
        for i in range(lu.columns.get_loc('1'), lu.columns.get_loc('49') + 1):
