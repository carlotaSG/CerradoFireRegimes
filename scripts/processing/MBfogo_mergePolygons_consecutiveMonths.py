...
def id_connectedComponents(gdf):
    """
    Functoin that gets a GeoDataFrame with polygons for two consecutive months and
    identifies those polygons that intersect one another within a certain buffer.
    
    The buffering distance is hard-coded to be 5 m (hence, 10 m in total).

    Parameters
    ----------
    gdf : GeoDataFrame
        GeoDataFrame containing the polygons from two consecutvie months to be merged.

    Returns
    -------
    gdf : GeoDataFrame
        Same GeoDataFrame as input but with an aditional column indicating with the same value
        all those polygons that are within 10 m of one another.
    ...
    # Casting this information as a column
...
def main():
    ...
    for year in list_years:
        ...
        year_fp = '../../data/tests/old/{}/MBfogo_c20_cerrado_polygons_{}_buff{}.{}'
        pol_fp = year_fp.format(year, year, buffer_label, 'shp')
        
        # Output directory
        # out_dir = '../../data/temp/{}_CM/'.format(year)
        out_dir = '../../data/tests/old/{}_CM/'.format(year)
