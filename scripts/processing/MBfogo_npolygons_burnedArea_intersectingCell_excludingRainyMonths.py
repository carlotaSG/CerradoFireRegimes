...
def main():
    ...
    prec_data_fp = in_dir + 'summary_tables/{}_year_periods/chirps_meanMonthlyPrec_grid{}_period{}.csv'
    
    polygons_fp = in_dir + 'MBfogo_c{}0_cerrado_polygons_b150_CM/MBfogo_c{}0_cerrado_polygons_{}.csv'.format(collection, collection, '{}')
    
    ba_data_fp = in_dir + 'summary_tables/{}_year_periods/burnedArea_fireCount_intersection_monthFromPixel_grid{}_period{}.csv'
    
    output_fp = in_dir + 'summary_tables/{}_year_periods/burnedArea_monthFromPixel_fireCount_assignedCell_exclude{}RainyMonths_grid{}_period{}.csv'
    ...
    # Working one period at a time
    for i in range(1, n_periods + 1):
        
        print('Working on period {}'.format(periods[i-1]))
        
        # Read average precipitation per month and cell for this period
        prec_data = pd.read_csv(prec_data_fp.format(n_years, grid_id, i))
        ...
        # Reading polygons for this period
        polygons = []
        ...
        # Counting polygons per cell and month in the period
        polygons = polygons.groupby(by = ['id','month']).size().reset_index(name = 'npolygons')
        polygons = polygons.sort_values(['id','month']).reset_index(drop = True)
        
        
        
        
        # Read burned area per cell, period and month
        ba_data = pd.read_csv(ba_data_fp.format(n_years, grid_id, i))
        ...
        # Sorting values
        ba_data = ba_data.sort_values(['id','month']).reset_index(drop = True)
        
        
        # Merge burned area and polygon data
        ba_data = ba_data.merge(polygons, on = ['id','month'], how = 'left')
        ...
        print('...saving file...\n')
        ba_data.to_csv(output_fp.format(n_years, nmonths, grid_id, i), index = False)
