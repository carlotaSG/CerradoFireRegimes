...
def calculate_monthDistribution(data, col_var):
    ...
    # UPDATE 18/11/2023: we also want to keep the relabelled months so that 
    # we can later do a temporal regression on the data to see if hte peak of
    # the season has actually receeded.
    m05_rel, m50_rel, m95_rel, length = calculate_sECDF(relabel_month_data)
    
    
    # 4. Convert these quantiles back to the original months
    m05 = adjustMonth(m05_rel - offset)
    m50 = adjustMonth(m50_rel - offset)
    m95 = adjustMonth(m95_rel - offset)
    
    
    return m05, m50, m95, length, m05_rel, m50_rel, m95_rel
...
def inverse_smoothedECDF_perCell(df, grouping_id, time_interval, var, exclude_nan = False):
    ...
    for (grouping, time_int), group in df.groupby( by = [grouping_id, time_interval]):
        ...
        if group[var].sum() == 0:
            ...
            m05, m50, m95, length, m05_rel, m50_rel, m95_rel = [np.nan]*7
        else:
            m05, m50, m95, length, m05_rel, m50_rel, m95_rel = calculate_monthDistribution(group, var)
        ...
        var_quant_ts.append(pd.DataFrame({
            ...
            'q05_month_sECDF_rel': m05_rel,
            'median_month_sECDF_rel': m50_rel,
            'q95_month_sECDF_rel': m95_rel
            }))
...
def save_fireSeasonCharacteristic(data, data_fp):
    ...
    # If it exists, read, merge data and save
        ...
        data_saved = data_saved.merge(data, on = ['id', 'year'], how = 'outer')
...
def main():
    ...
    dir_fp = '../../data/processed/summary_tables/annual/'

    
    data_fp = dir_fp + 'burnedArea_monthFromPixel_fireCount_assignedCell_exclude3RainyMonths_grid{}_year{}.csv'.format(grid_id, '{}')
    
    # Output filepath
    output_fp = dir_fp + 'fireSeason_measures_exclude3RainyMonths_grid{}_year{}.csv'
    
    
    # --------------- Reading data --------------------------------------------
    
    print('Reading data...')
    data = readFormat_dataYears(data_fp, start_year, end_year, grid_id,
                                ['area'], periods = False)#, thresh = cell_area_thresh)
    
    # ------------------ Processing data ------------------------------------
    
    # Calculating the quantiles per cell and period
    print('Calculating inverse sECDF...')
    inverseCDF = inverse_smoothedECDF_perCell(data, 'id', 'year', 'area', exclude_nan = False)
    
    
    # Subsetting columns of interest
    inverseCDF = inverseCDF[['id', 'year', 'q05_month_sECDF', 'median_month_sECDF', 
                             'q95_month_sECDF', 'q_length_sECDF', 'q05_month_sECDF_rel', 
                             'median_month_sECDF_rel', 'q95_month_sECDF_rel']]
    ...
    for year, data_subset in inverseCDF.groupby('year'):
        
        print('...for year {}'.format(year))
        
        
        # print(data_subset.head(20))
        save_fireSeasonCharacteristic(data_subset, output_fp.format(grid_id, year))
