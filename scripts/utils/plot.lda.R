...
#	centre[i,] <- apply(Fp[groups==i,], 2, mean) }  
	centre[i,] <- apply(Fp[groups==levels(as.factor(groups))[i],], 2, mean) }  # here
# print(centre)
...
if(plot.sites==1) {   # Plot symbols for the sites
	if(is.null(colour.vec)) {
		...
		points(Fp[,xax], Fp[,yax], pch=21, cex = 1, bg=colour.sel)
		}
...
#		cov <- cov(Fp[groups==i,c(xax,yax)])
		cov <- cov(Fp[groups==levels(as.factor(groups))[i], #here
		c(xax,yax)])
#		centre <- apply(Fp[groups==i,c(xax,yax)], 2, mean)
		centre <- apply(Fp[groups==levels(as.factor(groups))[i], #here
		c(xax,yax)], 2, mean)
