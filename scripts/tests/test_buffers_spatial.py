...
def id_connectedComponents(gdf, buff, month, last_W):
    ...
    # identifies those polygons that intersect one another with a certain buffer
...
def aggregate_connectedPolygons(gdf):
    # Function that aggregates same-group polygons into MultiPolygon. 
...
def main():
    ...
    # UPDATE 20/02/2023: modify script so that it works on the subset of original polygons
    # for a certain year (out of 1990, 2000, 2010, 2020) in folder ./data/tests/fire_polygons/individual
    #
    # This script is to e run for one of these years in the Linux cluster> on eyear per node
    #
    # output contians merged polygons according to various spatial buffers. Output is written 
    # to: ./data/tests/fire_polygons/spatial_buffer_<buffer in meters>
    #
    ...
    # Input file
    pol_fp = '../../data/tests/fire_polygons/individual/MBfogo_c10_cerrado_polygons_{}.shp'.format(year)
    
    
    # Output directory
    out_dir = '../../data/tests/fire_polygons/buff{}/'
    # Output file
    ...
    list_buffers = [500] # 15, 45, 75, 150, 150, 250, 
    
    # # For the annual version, we only merge those polygons that are touching one another
    # list_buffers = [5, 15]
    
    print('This node is working on year {} \n\n'.format(year))
    
    
    
    # Working on one buffer at a time
    for buffer in list_buffers:
        ...
        for month in range(1, 12 + 1):
            ...
            # to shapefile those that are not connected
            print('Storing non-connected components...')
            t.tic()
...
# -----------------------------------------------------------------------------
# In id_connectedComponents

# UPDATE 20/02/2023:  I think it should be safe to remove these lines of code
# TODO: Aftr trying to run this script in new setup - remove lines if ran correctly
...
#------------------------------------------------------------------------------
# This chunk was in function: aggregate_connectedPolygons
