...
def id_connectedComponents(gdf):
...
def main():
    ...
    # Input file
    pol_fp = '../../data/processed/MBfogo_c10_cerrado_polygons/MBfogo_c10_cerrado_polygons_{}.shp'
    
    
    # Output directory
    out_dir = '../../data/tests/fire_polygons/nobuff_consecutiveMonths/'
    # Output file
    shp_out_fp = out_dir + 'MBfogo_c10_cerrado_polygons_subset_{}_nobuff.shp'
    ...
    pols = gpd.read_file(pol_fp.format(year))
    pols = pols[pols['month'] == start_month]
    pols = pols[['ID','year','month','geometry']]
    ...
    for month in range(start_month + 1, end_month + 1):
        ...
        # polygons
        pol_month = gpd.read_file(pol_fp.format(year), include_fields=["ID", "year", "month"])
        pol_month = pol_month[['ID','year','month','geometry']]
        ...
        pols = id_connectedComponents(pols)
        ...
        if month - 1 == start_month:
            if not pols_notDissolve.loc[pols_notDissolve['month'] == month - 1, ['ID','year','month','geometry']].empty:
                # If we are working on the first round of month pairs, create output file and save
                pols_notDissolve.loc[pols_notDissolve['month'] == month - 1, ['ID','year','month','geometry']].to_crs(crs_original).to_file(shp_out_fp.format(year))
        else:
            if not pols_notDissolve.loc[pols_notDissolve['month'] == month - 1, ['ID','year','month','geometry']].empty:
                # Write these polygons appending them to output file
                pols_notDissolve.loc[pols_notDissolve['month'] == month - 1, ['ID','year','month','geometry']].to_crs(crs_original).to_file(shp_out_fp.format(year), mode = 'a')
    ...
    if not pols.empty:
        ...
        pols.to_crs(crs_original).to_file(shp_out_fp.format(year), mode = 'a')
...
#     new_ids = new_ids.rename(columns = {'ID': 'ID'})
    
#     # Merge this new datafrme back to lookup
#     lookup = lookup.merge(new_ids[['ID','connected_ids']], on = ['connected_ids'], how = 'left')
#     # print(lookup)
...
# def aggregate_connectedPolygons(gdf):
    ...
    # gdf = gdf[['ID','year','month','geometry']].dissolve(by = 'ID').reset_index()
    ...
    # mask = gdf.ID.duplicated(keep = False)
    
    # Hence, non-repeated elements
    # gdf_nonconnected = gdf.loc[~mask, ['ID','year','month','geometry']]
    
    # And repeated elements
    # gdf_connected = gdf.loc[mask, ['ID','year','month','geometry']]
    
    # del gdf
    
    # gdf = gdf[['ID','year','month','geometry']]
