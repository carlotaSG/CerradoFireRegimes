...
def main():
    ...
    for buffer in list_buffers:
        ...
        for month in range(start_month + 1, end_month + 1):
            ...
            # polygons
            pol_month = gpd.read_file(pol_fp.format(grid_id, cell_id, buffer), include_fields=["ID", "year", "month"])
            ...
            if month - 1 == start_month:
                ...
                pols_notDissolve.loc[pols_notDissolve['month'] == month - 1, ['ID','year','month','geometry']].to_crs(crs_original).to_file(shp_out_fp.format(grid_id, cell_id, buffer))
            else:
                # Write these polygons appending them to output file
                pols_notDissolve.loc[pols_notDissolve['month'] == month - 1, ['ID','year','month','geometry']].to_crs(crs_original).to_file(shp_out_fp.format(grid_id, cell_id, buffer), mode = 'a')
